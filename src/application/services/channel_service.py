    ...
    async def get_channel_prompt(self, channel: Channel, user_id: str) -> Optional[Any]:
        ...
        if not channel.selected_prompt_id:
            try:
                ...
                return None
            if not prompt:
                logger.info("Channel %s has no selected_prompt_id and no prompts exist for user %s.", channel.id, user_id, extra={"class": self.__class__.__name__, "method": inspect.currentframe().f_code.co_name},)
                return None
            if getattr(prompt, "user_id", None) != channel.user_id:
                logger.info("Fallback prompt %s does not belong to user %s (channel user_id=%s).", getattr(prompt, "id", None), user_id, channel.user_id, extra={"class": self.__class__.__name__, "method": inspect.currentframe().f_code.co_name},)
                return None
            logger.info("Channel %s has no selected_prompt_id; using fallback prompt %s for user %s and channel %s", channel.id, getattr(prompt, "id", None), user_id, channel.id, extra={"class": self.__class__.__name__, "method": inspect.currentframe().f_code.co_name},)
            return prompt
        ...
        if not prompt:
            ...
            try:
                ...
                return None
            if not prompt:
                logger.info("No fallback prompts exist for user %s and channel %s.", user_id, channel.id, extra={"class": self.__class__.__name__, "method": inspect.currentframe().f_code.co_name},)
                return None
            
        if getattr(prompt, "user_id", None) != channel.user_id:
            logger.info("Resolved prompt %s does not belong to user %s (channel user_id=%s).", getattr(prompt, "id", None), user_id, channel.user_id, extra={"class": self.__class__.__name__, "method": inspect.currentframe().f_code.co_name},)
            return None
        
        logger.info("Selected prompt %s successfully retrieved for user %s and channel %s", getattr(prompt, "id", None), user_id, channel.id, extra={"class": self.__class__.__name__, "method": inspect.currentframe().f_code.co_name},)
