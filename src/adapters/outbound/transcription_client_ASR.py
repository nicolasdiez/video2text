...
class YouTubeTranscriptionClientASR(TranscriptionPort):
    ...
    def _download_audio_stream(self, video_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            logger.info(
                "yt-dlp extracted info for video %s",
                video_id,
                extra={"class": self.__class__.__name__, "method": method_name},
            )
        except Exception as exc:
            logger.exception(
                "yt-dlp failed to extract info for video %s: %s",
                video_id,
                str(exc),
                extra={"class": self.__class__.__name__, "method": method_name},
            )
        ...
        if not audio_url:
            logger.warning(
                "No audio URL found for video %s",
                video_id,
                extra={"class": self.__class__.__name__, "method": method_name},
            )
            return None
        ...
        logger.info(
            "Selected audio format for video %s",
            video_id,
            extra={"class": self.__class__.__name__, "method": method_name},
        )
        ...
        try:
            ...
            logger.exception(
                "Failed to start ffmpeg for video %s: %s",
                video_id,
                str(exc),
                extra={"class": self.__class__.__name__, "method": method_name},
            )
        ...
        try:
            ...
            proc.kill()
            logger.exception(
                "Error while running ffmpeg for video %s: %s",
                video_id,
                str(exc),
                extra={"class": self.__class__.__name__, "method": method_name},
            )
        ...
        if proc.returncode != 0:
            logger.warning(
                ...
                proc.returncode,
                video_id,
                (stderr_data.decode("utf-8", errors="replace")[:400] if stderr_data else ""),
                extra={"class": self.__class__.__name__, "method": method_name},
            )
            return None

        logger.info(
            "ffmpeg produced %d bytes of WAV for video %s",
            len(stdout_data) if stdout_data else 0,
            video_id,
            extra={"class": self.__class__.__name__, "method": method_name},
        )
        return stdout_data
    ...
    def _load_audio_from_bytes(self, wav_bytes: bytes) -> Tuple[Optional[np.ndarray], Optional[int]]:
        ...
        try:
            ...
            logger.info(
                "Decoded WAV into numpy array (samples=%d, sr=%d)",
                data.shape[0],
                sr,
                extra={"class": self.__class__.__name__, "method": method_name},
            )
